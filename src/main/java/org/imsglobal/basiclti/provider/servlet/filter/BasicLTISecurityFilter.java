...
public class BasicLTISecurityFilter extends GenericFilterBean {
	...
	private BasicLtiContext buildBasicLtiContext(OAuthMessage requestMessage) throws IOException {
		BasicLtiContext context = new BasicLtiContext();
		
		LOG.info("requestMessage: user_id = " + requestMessage.getParameter("user_id"));
		...
		// BEGIN CU - wc2442
		// use custom_canvas_user_login_id as lis_person_source_id
		String canvas_user_login_id = requestMessage.getParameter("custom_canvas_user_login_id");
		if(canvas_user_login_id != null) {
			String login_id = canvas_user_login_id.replaceAll("@\\S+", "");
			context.getLisPerson().setSourcedId(login_id);
			LOG.debug("lis_person_scource_id: canvas_user_login_id = " + login_id);
		}
		// END CU - wc2442
	...
	private void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (!isBasicLtiRequest && isBasicLtiSet(session)) {
			LOG.debug("Not a BasicLTI Request.");
			chain.doFilter(request, response);
		} else {
			if (isBasicLtiRequest) {
				// handle
				LOG.debug("BasicLti Request");
	...
	protected ConsumerSecretService getConsumerSecretService() throws ServletException {
		...
		ConsumerSecretService bean = (ConsumerSecretService)context.getBean(ConsumerSecretService.class.getName(), ConsumerSecretService.class);
		if (bean == null) {
